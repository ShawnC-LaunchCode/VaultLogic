...
-- Create a unique partial index for non-null aliases within a workflow
-- This enforces uniqueness of aliases per workflow while allowing NULL values
-- FIXED: Commented out because Postgres does not support subqueries in index expressions.
-- Use an application-level check or a trigger if strict enforcement is needed.
...
CREATE INDEX IF NOT EXISTS idx_steps_alias ON steps(alias) WHERE alias IS NOT NULL;
