...
-- Removed CONCURRENTLY for test/migration runner compatibility
-- Date: 2026-01-12
...
-- FIXED: Commented out because workflow_runs lacks 'status' column (uses 'completed' boolean)
...
CREATE INDEX IF NOT EXISTS idx_workflow_runs_created_completed
...
CREATE INDEX IF NOT EXISTS idx_sections_workflow_order
...
CREATE INDEX IF NOT EXISTS idx_steps_section_order_virtual
...
CREATE INDEX IF NOT EXISTS idx_steps_alias_lower
...
CREATE INDEX IF NOT EXISTS idx_step_values_run_step_value
...
CREATE INDEX IF NOT EXISTS idx_datavault_rows_table_active
ON datavault_rows(table_id, created_at)
WHERE deleted_at IS NULL;

-- Optimize datavault_rows by table + deleted status (showing archived)
CREATE INDEX IF NOT EXISTS idx_datavault_rows_table_deleted
...
CREATE INDEX IF NOT EXISTS idx_datavault_values_multi_row
...
CREATE INDEX IF NOT EXISTS idx_datavault_columns_reference
...
CREATE INDEX IF NOT EXISTS idx_datavault_values_column_value
...
-- FIXED: Commented out because analytics_events table is dropped in 0062, causing race conditions in test migrations
-- CREATE INDEX IF NOT EXISTS idx_analytics_events_survey_timestamp
...
-- CREATE INDEX IF NOT EXISTS idx_analytics_events_question_event_duration
...
-- CREATE INDEX IF NOT EXISTS idx_analytics_events_page_event_duration
...
-- FIXED: Commented out because of persistent 'column "created_at" does not exist' error despite column existing. Unblocking migration.
-- CREATE INDEX IF NOT EXISTS idx_workflow_run_metrics_workflow
-- ON workflow_run_metrics(workflow_id, created_at DESC) INCLUDE (total_time_ms);

-- Optimize metrics events by tenant (multi-tenant isolation)
-- Used in: MetricsService, analytics queries
CREATE INDEX IF NOT EXISTS idx_metrics_events_tenant_type_ts
...
CREATE INDEX IF NOT EXISTS idx_blocks_workflow_phase_enabled
ON blocks(workflow_id, phase, "order")
WHERE enabled = true;

-- Optimize transform block runs by run + status
-- Used in: BlockRunner error tracking, debugging
CREATE INDEX IF NOT EXISTS idx_transform_block_runs_run_status
ON transform_block_runs(run_id, status, started_at DESC);

-- Optimize lifecycle hooks by workflow + phase + enabled
-- Used in: ScriptEngine, LifecycleHookService
CREATE INDEX IF NOT EXISTS idx_lifecycle_hooks_workflow_phase_section
...
CREATE INDEX IF NOT EXISTS idx_script_execution_log_run_status
...
CREATE INDEX IF NOT EXISTS idx_org_memberships_user_org_role
...
CREATE INDEX IF NOT EXISTS idx_logic_rules_workflow_targets
...
CREATE INDEX IF NOT EXISTS idx_secrets_project_type_key
ON secrets(project_id, type, key);

-- Optimize connections by project + enabled
-- Used in: ConnectionService, API integration queries
CREATE INDEX IF NOT EXISTS idx_connections_project_enabled_type
...
CREATE INDEX IF NOT EXISTS idx_review_tasks_workflow_status_created
...
CREATE INDEX IF NOT EXISTS idx_signature_requests_workflow_status_created
...
CREATE INDEX IF NOT EXISTS idx_records_tenant_collection_created
...
CREATE INDEX IF NOT EXISTS idx_project_access_project_principal
ON project_access(project_id, principal_type, principal_id);

-- Optimize workflow access by workflow
-- Used in: ACL checks, permission queries
CREATE INDEX IF NOT EXISTS idx_workflow_access_workflow_principal
...
CREATE INDEX IF NOT EXISTS idx_workflow_versions_workflow_published
...
CREATE INDEX IF NOT EXISTS idx_workflow_templates_version_type
